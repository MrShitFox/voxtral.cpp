...
const steps = [
  ...
  { name: "incremental-encoder", command: "npm", args: ["run", "test:incremental-encoder:gpu"] },
];
...
try {
  for (const step of steps) {
    console.log(`[incremental-encoder-acceptance] ${step.name}`);
    const result = await runProcess(step.command, step.args, {
      cwd: step.cwd ?? nodeCwd,
      timeoutMs: step.name.includes("build") || step.name === "incremental-encoder" ? 1_800_000 : 600_000,
  ...
  const artifact = await writeArtifactBundle({
    config,
    testName: "incremental-encoder-acceptance-summary",
    backend: "Vulkan",
    command: "npm run acceptance:incremental-encoder",
    result: summary,
  });
  console.log(`[incremental-encoder-acceptance] summary: ${artifact.directory}`);
}
