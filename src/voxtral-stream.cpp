...
voxtral_status feed_common(voxtral_stream * s, const void * ptr, size_t count, Convert convert) {
    ...
    if (s->lifecycle.state == voxtral_stream_state::created) {
        s->lifecycle.state = voxtral_stream_state::running;
    }
    if (backpressured) {
    ...
    const voxtral_stream_params  & params)
{
    ...
    if (model != nullptr) {
        ...
        if (!ctx) {
            ...
            s->lifecycle.state = voxtral_stream_state::failed;
...
voxtral_status voxtral_stream_warmup_internal(voxtral_stream * stream) {
    ...
    if (stream->lifecycle.warmup_complete) return voxtral_status::ok;
    if (stream->lifecycle.state != voxtral_stream_state::created ||
    ...
    if (!ensure_frontend(stream) || !ensure_encoder(stream) || !stream->frontend.enc) {
        if (stream->lifecycle.last_status == voxtral_status::ok) {
...
voxtral_status voxtral_stream_finish_internal(voxtral_stream * stream) {
    ...
    stream->lifecycle.state = voxtral_stream_state::finishing;
    ...
    if (stream->lifecycle.total_samples_received == 0) {
        ...
        stream->lifecycle.state = voxtral_stream_state::completed;
        clear_error(stream);
        return voxtral_status::ok;
    }
    ...
    if (stream->ctx == nullptr || stream->frontend.mel_fe == nullptr) {
        ...
        emit_error(stream, voxtral_status::backend_error, stream->lifecycle.last_error);
        stream->lifecycle.state = voxtral_stream_state::failed;
        return voxtral_status::backend_error;
    }
...
voxtral_status voxtral_stream_reset_internal(voxtral_stream * stream) {
    ...
    stream->lifecycle.state = voxtral_stream_state::created;
    clear_error(stream);
    return voxtral_status::ok;
}
...
voxtral_status voxtral_stream_cancel_internal(voxtral_stream * stream) {
    ...
    stream->lifecycle.state = voxtral_stream_state::cancelled;
...
voxtral_stream_state voxtral_stream_get_state(const voxtral_stream * s) {
    return s ? s->lifecycle.state : voxtral_stream_state::failed;
}
voxtral_status voxtral_stream_last_status(const voxtral_stream * s) {
    return s ? s->lifecycle.last_status : voxtral_status::invalid_argument;
}
